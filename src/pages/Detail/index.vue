  ...
  methods: {
    ...
    async addShopCart() {
      try {
        ...
        alert("添加购物车成功")
        /*
            详情页需要商品展示，因此需要将数据传递过去，当数据太大时选择用存储方案,
            localstorage（永久存储）和sessionStorage（临时存储），此处选择临时存储
            当数据很少只有数字之类的选择路由传参，query方式更方便
        */
       //  传递的参数需要转换，否则会调用tostring，数据就会变成两个object
